...
export default async function Page({ params }: Params) {
  const { quoteId, slug } = await params;

  // 1. FETCH QUOTE DATA
  const quote = await getQuote(quoteId);
  if (!quote || !quote.customer || !quote.booking || !slug) notFound();
  ...
  const isAuthorizedCustomer = Boolean(
    sessionCustomerId && sessionCustomerId === quote.customer.id,
  );

  // 2. Fallback: Logged in specifically using this quote's booking ID
  const isAuthorizedBooking = Boolean(
    sessionHuelineId &&
    sessionHuelineId === quote.booking.huelineId?.toLowerCase(),
  );
  ...
  // 4. RENDER PAGE
  return (
    <QuoteProvider slug={slug} quoteId={quoteId}>
      <SingleQuoteIdPage quote={quote} isOwner={isOwner} />
    </QuoteProvider>
  );
}
