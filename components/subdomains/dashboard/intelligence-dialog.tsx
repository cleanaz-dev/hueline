          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-xs text-slate-500 font-medium">
              <span className="flex items-center gap-1.5">
                ...
                {sortedCalls.length} Interaction
          ...
          <div className="space-y-8">
            {sortedCalls.map((call, index) => {
              ...
              const outcomeStyle = outcome
                ...
                : outcomeConfig.NEUTRAL;
              const customFields =
                (intel?.customFields as Record<string, any>) || {};
              ...
              const validEntries = Object.entries(customFields).filter(
                ([_, val]) => {
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-200">
                      ...
                      <div className="px-4 py-3 border-b border-slate-50 flex flex-wrap justify-between items-center gap-2 bg-slate-50/30">
                        <div className="flex items-center gap-2.5">
                          <span className="font-semibold text-sm text-slate-900">
                            {formatCallReason(
                              intel?.callReason || "General Inquiry"
                            )}
                          </span>
                          {outcome && (
                            <span
                      ...
                      <div className="p-4 space-y-4">
                        {/* 1. AUDIO PLAYER */}
                        {call.audioUrl && (
                          <MinimalAudioPlayer url={call.audioUrl} />
                        )}
                        ...
                        {intel?.costBreakdown && (
                          <div className="flex gap-3">
                            <div className="shrink-0 mt-0.5">
                              <div className="w-6 h-6 rounded-full bg-indigo-50 flex items-center justify-center">
                                <SquareFunction className="size-3.5 text-cyan-500" />
                              </div>
                            </div>
                            <p className="text-sm text-slate-600 leading-relaxed">
                              {intel.costBreakdown}
