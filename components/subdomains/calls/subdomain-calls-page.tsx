...
export default function SubdomainCallsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"ALL" | "RISK" | "OPPORTUNITY">(
    "ALL"
  );
  ...
  const handlePlayPause = async (callSid: string) => {
    // If already playing this one, just stop
    if (playingId === callSid) {
      setPlayingId(null);
      return;
    }

    // If we already have a valid presigned URL, use it
    if (presignedUrls[callSid]) {
      setPlayingId(callSid);
      return;
    }

    // Fetch a new presigned URL
    try {
      setIsLoadingAudio(callSid);
      const response = await fetch(
        `/api/subdomain/${subdomain.slug}/call/${callSid}`
      );
      const data = await response.json();

      if (data.url) {
        setPresignedUrls((prev) => ({ ...prev, [callSid]: data.url }));
        setPlayingId(callSid);
      }
    } catch (error) {
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const reason =
            row.original.intelligence?.callReason?.replace("_", " ") || "OTHER";
          return (
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <p
            className="text-xs text-gray-600 line-clamp-2 max-w-46 font-normal leading-relaxed truncate"
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center">
              ...
              {/* Audio Tag uses the cached presigned URL */}
              {playingId === call.callSid && (
                <audio
                  src={presignedUrls[call.callSid]}
                  autoPlay
  ...
  const activeData = useMemo(() => {
    switch (activeTab) {
      case "RISK":
        return subdomain.calls.filter(
          (c) => c.intelligence?.callOutcome === "NEGATIVE"
        );
      case "OPPORTUNITY":
        return subdomain.calls.filter(
          (c) =>
        ...
        <div>
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 bg-zinc-100 rounded-2xl  items-center justify-center border border-zinc-200 shadow-sm hidden md:flex">
              <Phone className="w-7 h-7 text-zinc-700" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Calls</h2>
              <p className="text-sm text-gray-500 mt-1">
                Analyze conversations, identify risks, and spot revenue
                opportunities.
              </p>
            </div>
          </div>
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    ...
                    return (
                      <td
                        key={cell.id}
                        className={cn("px-6 py-4 whitespace-nowrap", metaClass)}
                      >
                        {flexRender(
          ...
          table
            ...
            .rows.map((row) => (
              <MobileCallCard
                ...
                onPlayToggle={() =>
                  setPlayingId(
                    playingId === row.original.id ? null : row.original.id
                  )
                }
              />
            ))
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-all duration-200">
      ...
      {isExpanded && (
        <div className="bg-gray-50/50 border-t border-gray-100 px-4 py-3 text-sm animate-in slide-in-from-top-2 duration-200">
          ...
          {/* Summary Text */}
          <div>
            <h5 className="text-[10px] uppercase text-gray-400 font-bold tracking-wider mb-1">
              Summary
            </h5>
            <p className="text-gray-700 leading-relaxed text-xs">
              {data.intelligence?.callSummary ||
