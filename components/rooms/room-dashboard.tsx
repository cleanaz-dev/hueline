...
export default function RoomsDashboard() {
  ...
  const handleCreateRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "project") {
        const slug = selectedBooking?.name.toLowerCase().replace(/\s+/g, "") || "project";
        ...
        const slug = quickClientName.toLowerCase().replace(/\s+/g, "") || "quick";
        roomId = `${slug}-${dateStr}-${uuidv4().slice(0, 4)}`;
        payload = {
  ...
  return (
    <div className="container mx-auto max-w-6xl py-10 px-4 md:px-6 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-2">
            Bridge the gap between a lead and a contract with a virtual walkthrough.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-stretch">
        ...
        <form onSubmit={handleCreateRoom} className="lg:col-span-3 flex flex-col h-full">
          <Card className="border-zinc-200 shadow-sm flex-1 flex flex-col">
              ...
              {mode === "project" ? (
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
                  <div className="space-y-2">
                    <Label htmlFor="booking-select" className="text-zinc-700">Active Project</Label>
                    <Select value={selectedBookingId} onValueChange={setSelectedBookingId}>
                      <SelectTrigger id="booking-select" className="h-12 border-zinc-300">
                        ...
                        {subdomain.bookings?.map((booking) => (
                          <SelectItem key={booking.id} value={booking.id}>
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-zinc-900">{booking.name}</span>
                              <span className="text-[10px] uppercase text-zinc-400 tracking-wider">ID: {booking.huelineId}</span>
                            </div>
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input className="border-zinc-300" placeholder="e.g. Living Room Walkthrough" value={projectName} onChange={(e) => setProjectName(e.target.value)} />
                  </div>
                </div>
              ) : (
                /* QUICK SESSION FORM */
            ...
            <div className="p-6 pt-0 mt-auto">
              <Button
                ...
                disabled={isCreating || (mode === 'project' ? !projectName : !quickRoomName)}
              >
                {isCreating ? "Initializing..." : mode === "project" ? "Launch Linked Room" : "Start Quick Session"}
              ...
              <Card className="bg-card text-foreground border-none sticky top-6 h-full shadow-sm">
                <CardHeader className="border-b border-zinc-100">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-primary text-xs font-mono">{selectedBooking.huelineId}</span>
                  </div>
                  <CardTitle className="text-2xl pt-2">{selectedBooking.name}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-6 pt-6">
                  ...
                  {selectedBooking.estimatedValue !== undefined && selectedBooking.estimatedValue !== null && (
                    <div className="p-4 rounded-2xl bg-gray-50 border">
                      <p className="text-xs text-primary uppercase font-bold tracking-widest">Potential Value</p>
                      <p className="text-3xl">${selectedBooking.estimatedValue.toLocaleString()}</p>
                    </div>
                  )}
                  {selectedBooking.projectScope && selectedBooking.projectScope.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs text-zinc-500 uppercase font-bold tracking-widest">Planned Scope</p>
                      <div className="flex flex-wrap gap-2">
                        {selectedBooking.projectScope.map((scope, idx) => (
                          <span key={idx} className="px-2 py-1 bg-zinc-100 rounded text-[11px] font-medium text-zinc-700">
                            {scope}
                          </span>
                        ))}
                      </div>
            ...
            <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-100 h-full shadow-sm">
              ...
              <CardContent className="space-y-6 pt-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-blue-400 uppercase font-bold">Date</p>
                ...
                <div className="p-4 rounded-2xl bg-white/60 border border-blue-100">
                  <p className="text-xs text-blue-400 uppercase font-bold tracking-widest">Estimated Value</p>
                  <p className="text-3xl text-zinc-300 font-light">---</p>
                  <p className="text-[10px] text-zinc-400 mt-1">Calculated after room scope analysis</p>
                </div>

                <div className="space-y-2 opacity-60">
                  <p className="text-xs text-blue-400 uppercase font-bold tracking-widest">Scope Intelligence</p>
                  <div className="flex flex-wrap gap-2">
