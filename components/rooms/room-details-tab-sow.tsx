...
export function RoomDetailsTabSow({
  ...
  const handleAddItem = (newItem: ScopeItem) => {
    const itemWithId = { ...newItem, timestamp: newItem.timestamp || new Date().toISOString() };
    executeUpdate([...items, itemWithId]);
    setIsAddOpen(false);
  };
  ...
  const handleDeleteItem = () => {
    ...
    const updatedList = items.filter((i) => i.timestamp !== selectedItem.timestamp);
    executeUpdate(updatedList);
  ...
  const imageMap = useMemo(() => {
    ...
    items.filter((i) => i.type === ScopeType.IMAGE && i.image_id).forEach((img) => map.set(img.image_id!, img));
    return map;
  }, [items]);

  const groupedByArea = useMemo(() => {
    const filtered = activeArea === "ALL" ? items : items.filter((i) => i.area === activeArea);
      ...
      <ScrollArea className="h-full bg-zinc-50/50">
        <div className="p-4 space-y-6">
          <div className="flex justify-end">
            <Button size="sm" variant="outline" onClick={() => openAddDialog()} className="gap-2">
          ...
          {items.length === 0 && (
            <div className="flex flex-col items-center justify-center py-10 opacity-50">
              ...
              <p className="text-sm text-zinc-500 font-medium">No scope items recorded.</p>
              <Button variant="link" onClick={() => openAddDialog()} className="mt-2">Add your first item</Button>
            </div>
          )}
          ...
          {Object.entries(groupedByArea).map(([areaName, areaItems]) => {
            ...
            areaItems.forEach((i) => {
              if (i.type === ScopeType.IMAGE) return;
              if (!itemsByCategory[i.type]) itemsByCategory[i.type] = [];
              itemsByCategory[i.type].push(i);
            });

            return (
              <div key={areaName} className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden">
                {/* HEADER */}
                <div className="relative border-b border-zinc-100">
                  ...
                  <div className={cn("relative z-10 p-4 flex items-end justify-between", coverUrl ? "pt-20 text-white" : "bg-zinc-50/80 text-zinc-900")}>
                    <div>
                      {allKeys.length > 1 && (
                        <div className={cn("flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider backdrop-blur-md mb-1 w-fit", coverUrl ? "bg-black/40 text-white border border-white/20" : "bg-zinc-100 text-zinc-500 border border-zinc-200")}>
                    ...
                    <Badge variant="outline" className={cn("text-[10px] border h-6", coverUrl ? "bg-white/10 border-white/30 text-white backdrop-blur-md" : "bg-white text-zinc-500 border-zinc-200")}>
                          ...
                          <div key={idx} className="aspect-square rounded-lg overflow-hidden border border-zinc-200 bg-zinc-100 hover:ring-2 hover:ring-blue-500 transition-all cursor-pointer group relative">
                            {url ? (
                              <img src={url} alt={`photo ${idx}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                               ...
                               <div className="w-full h-full flex items-center justify-center"><Loader2 className="w-4 h-4 animate-spin text-zinc-300" /></div>
                            )}
                          </div>
                        );
                      })}
                ...
                {CATEGORY_ORDER.map((cat) => {
                  ...
                  return (
                    <div key={cat} className="border-b border-zinc-100 last:border-b-0">
                      <div className="px-3 py-2 flex items-center justify-between gap-2 group/header">
                        ...
                        <CirclePlus onClick={() => openAddDialog(areaName, cat)} className={cn("w-3.5 h-3.5 cursor-pointer hover:opacity-70 mr-1 transition-opacity opacity-0 group-hover/header:opacity-100", config.color)} />
                      </div>
                                ...
                                <td className="px-3 py-2 text-right w-[60px]">
                                  <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                      <Button variant="ghost" size="sm" className="h-6 w-6 p-0 opacity-0 group-hover/row:opacity-100 transition-opacity"><MoreHorizontal className="size-3" /></Button>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent align="end" className="w-32">
                                      <DropdownMenuItem onClick={() => { setSelectedItem(item); setIsEditOpen(true); }} className="text-xs cursor-pointer"><Pencil className="mr-2 size-3 hover:text-white" /> Edit</DropdownMenuItem>
                                      <DropdownMenuSeparator />
                                      <DropdownMenuItem onClick={() => { setSelectedItem(item); setIsDeleteOpen(true); }} className="text-xs text-red-600 cursor-pointer" variant="destructive"><Trash2 className="mr-2 size-3" /> Delete</DropdownMenuItem>
                                    </DropdownMenuContent>
                                  </DropdownMenu>
                                </td>
                              </tr>
