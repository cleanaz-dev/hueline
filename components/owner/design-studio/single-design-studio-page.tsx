...
import { DesignProject, Customer } from "@/app/generated/prisma";
...
export default function SingleDesignStudio({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Revalidate to get updated status from server
      mutateProject();
    } catch (error) {
    ...
    <div className="flex w-full gap-6 bg-transparent p-6 font-sans text-zinc-900">
      ...
      <section className="flex flex-1 flex-col gap-4">
        ...
        <div className="relative flex w-full flex-1 max-h-[65vh] min-h-[400px] items-center justify-center overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm ring-1 ring-zinc-100/50">
          ...
          {/* Generating Overlay */}
          {isGeneratingProjectImage && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-zinc-900/70 backdrop-blur-sm rounded-2xl">
          ...
          {imageUrl ? (
            ...
            <div className="relative h-full w-full">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={previewUrl} alt="Preview" className="h-full w-full object-contain bg-zinc-50" />
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-900/60 backdrop-blur-[2px] transition-all">
                {isUploading ? (
                  <div className="flex flex-col items-center text-center">
                    ...
                    <h3 className="text-sm font-bold text-white tracking-wide">Uploading to Workspace...</h3>
                  </div>
                ) : (
      ...
      {/* --- Right Column: Color Palette Component --- */}
      <DesignStudioPalette 
