...
export const RoomProvider = ({
    ...
    const handleDataReceived = (payload: Uint8Array, participant?: RemoteParticipant) => {
      try {
        ...
        if (data.type === "SCOPE_UPDATE") {
          mutate((curr: any) => ({
            ...curr,
            scopeData: { items: [data.item, ...(curr?.scopeData?.items || [])] }
          }), false);
        }
        ...
        if (data.type === "SCOPE_REMOVE") {
          mutate((curr: any) => ({
            ...curr,
            scopeData: { items: curr?.scopeData?.items?.filter((i: ScopeItem) => i.id !== data.id) || [] }
          }), false);
        }
  ...
  const handleChunkAnalysis = async (text: string) => {
    if (!isPainter) return;
    try {
      const res = await fetch(`/api/subdomain/${slug}/room/${room.name}/analyze-chunk`, {
        method: 'POST',
  ...
  const removeScopeItem = useCallback((id: string) => {
    ...
    mutate((curr: any) => ({
      ...curr,
      scopeData: { items: curr?.scopeData?.items?.filter((i: ScopeItem) => i.id !== id) || [] }
    }), false);
  ...
  const saveAndEndSession = useCallback(async () => {
    ...
    try {
      ...
      await fetch(`/api/subdomain/${slug}/room/${room.name}/end`, { method: 'POST' });
      room.disconnect();
  ...
  const toggleTranscription = useCallback(async () => {
    ...
    try {
      ...
      const connection = deepgram.listen.live({
        model: "nova-2", language: "en-US", smart_format: true, encoding: "linear16", sample_rate: 16000, interim_results: true
      });
