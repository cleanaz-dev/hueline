...
interface OwnerContextValue {
  ...
  sendEmail: (customerId: string, body: string, subject?: string) => Promise<boolean>;
...
export function OwnerProvider({
  ...
  const sendSMS = async (customerId: string, body: string): Promise<boolean> => {
  ...
  const sendEmail = async (customerId: string, body: string, subject?: string): Promise<boolean> => {
  ...
  const fetchAiSuggestion = async (customerId: string): Promise<void> => {
    setIsAiLoading(true);
    try {
      const res = await fetch(`/api/subdomain/${subdomain.slug}/ai-suggestion?customerId=${customerId}`);
  ...
  const generateImage = async (
    ...
    try {
      const res = await fetch(`/api/subdomain/${subdomain.slug}/jobs/create-image-job`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prospectId, mediaUrl, ...payload }),
      });
      if (res.ok)  setGenerateImageSuccess(
